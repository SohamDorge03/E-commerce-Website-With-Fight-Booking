...
require_once __DIR__ . '/Admin/vendor/autoload.php'; // Include the mPDF library

if(isset($_POST['generate_pdf'])) {
    // Assuming $booking_id is the variable representing the booking ID
    $booking_id = 1; // Example: Get the booking ID from the URL parameter
    ...
    $sql = "SELECT 
        ...
        f.dest_date, 
        f.dest_time, 
        a1.airport_code AS dep_airport_code, 
        a2.airport_code AS arr_airport_code, 
        a.airline_name,
    ...
    INNER JOIN 
        airports a1 ON f.dep_airport_id = a1.airport_id
    INNER JOIN 
        airports a2 ON f.arr_airport_id = a2.airport_id
    INNER JOIN 
        airlines a ON bf.airline_id = a.airline_id
    LEFT JOIN 
        passenger p ON bf.booking_id = p.booking_id
    WHERE 
        bf.booking_id = $booking_id"; // Modify the query to filter by booking ID
    ...
    if ($conn->error) {
        ...
        if ($result && $result->num_rows > 0) {
            while($row = $result->fetch_assoc()) {
                ...
                <div class="box">
                          <div class="ticket">
                            <span class="airline">' . $row["airline_name"] . '</span>
                            <span class="airline airlineslip">' . $row["airline_name"] . '</span>
                            <span class="boarding">Boarding pass</span>
                            <div class="content">
                              <span class="jfk">' . $row["dep_airport_code"] . '</span>
                              ...
                              <span class="sfo">' . $row["arr_airport_code"] . '</span>
                              
                              <span class="jfk jfkslip">' . $row["dep_airport_code"] . '</span>
                              <span class="plane planeslip"><svg clip-rule="evenodd" fill-rule="evenodd" height="50" width="50" image-rendering="optimizeQuality" shape-rendering="geometricPrecision" text-rendering="geometricPrecision" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg"><g stroke="#222"><line fill="none" stroke-linecap="round" stroke-width="30" x1="300" x2="55" y1="390" y2="390"/><path d="M98 325c-9 10 10 16 25 6l311-156c24-17 35-25 42-50 2-15-46-11-78-7-15 1-34 10-42 16l-56 35 1-1-169-31c-14-3-24-5-37-1-10 5-18 10-27 18l122 72c4 3 5 7 1 9l-44 27-75-15c-10-2-18-4-28 0-8 4-14 9-20 15l74 63z" fill="#222" stroke-linejoin="round" stroke-width="10"/></g></svg></span>
                              <span class="sfo sfoslip ">' . $row["arr_airport_code"] . '</span>
                              <div class="sub-content">
                                <span class="watermark">' . $row["airline_name"] . '</span>
                                <span class="name">PASSENGER NAME<br><span class="data">' . $row["passenger_name"] . '</span></span>
                                <span class="flight">FLIGHT N&deg;<br><span class="data">' . $row["flight_code"] . '</span></span>
                                <span class="gate">GATE<br><span class="data">' . $row["gateno"] . '</span></span>
                                <span class="seat">SEAT<br><span class="data">' . $row["seatno"] . '</span></span>
                                <span class="boardingtime">BOARDING TIME<br><span class="data">' . $row["boarding_time"] . '</span></span>
                                <span class="flight flightslip">FLIGHT N&deg;<br><span class="data">' . $row["flight_code"] . '</span></span>
                                <span class="seat seatslip">SEAT<br><span class="data">' . $row["seatno"] . '</span></span>
                                <span class="name nameslip">PASSENGER NAME<br><span class="data">' . $row["passenger_name"] . '</span></span>
                              </div>
                            </div>
                ...
                $mpdf->Output('boarding_pass.pdf', 'I'); // 'F' for file
            }
        } else {
            echo "0 results";
        }
